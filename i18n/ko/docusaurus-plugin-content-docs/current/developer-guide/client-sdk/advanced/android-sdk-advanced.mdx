...
Notifly Android SDK에서는 자체적으로 알림 권한을 요청하지 않습니다.

사용자 알림 수신 동의는 Notifly Android SDK 설치와 무관하게 앱 내에서 직접 구현하셔야 하며, 특히 Android 13 TIRAMISU(API Level 33) 이상에서는 명시적으로 사용자에게 `POST_NOTIFICATIONS` 권한을 획득해야 합니다. 푸시 알림 수신 동의를 사용자에게 런타임에 요청하는 가이드는 [런타임 권한 요청](https://developer.android.com/training/permissions/requesting?hl=ko)을 참고해 주세요.

```xml title="AndroidManifest.xml for notification permission"
<manifest ...>
