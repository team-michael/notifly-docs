...
`OSNotificationClickEvent` 의 `OSNotificationClickEvent` 멤버에 접근하여, 사용자가 클릭한 푸시에 대한 정보를 확인할 수 있습니다.
...
| `customData`            | `Map<String, dynamic>?` | FCM 푸시 메시지의 `data` 필드에 포함된 키-값 쌍의 커스텀 데이터입니다. 노티플라이 콘솔에서 설정할 수 있습니다.          | 1.5.0 |
| `imageUrl`              | `String?`               | 이미지형 푸시 메시지에 포함된 이미지 URL 입니다.                                                                        | 1.5.0 |
| `importance`            | `Importance`            | `Importance.high`, `Importance.normal`, `Importance.low` 중 하나의 값입니다.                                            | 1.5.0 |
| `notiflyMessageId`      | `String?`               | Notifly 서버에서 임의로 생성한 메시지 ID입니다.                                                                         | 1.5.0 |
| `rawPayload`            | `Map<String, dynamic>?` | FCM data의 가공되지 않은 payload입니다. JSON을 stringify한 형태로 반환됩니다.                                           | 1.5.0 |
| `sentTime`              | `int?`                  | FCM 서버에서 푸시 알림을 발송한 시간입니다.                                                                             | 1.5.0 |
