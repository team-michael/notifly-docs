...
- username은 노티플라이 웹페이지의 [설정 페이지](https://notifly.tech/console/settings?environment=0)의 API, SDK 인증 정보 섹션에서 확인하실 수 있습니다. (initialize method에서 사용하는 값과 동일합니다.)
...
target 'NotiflyNotificationExtension' do
  ...
  pod 'notifly_sdk', '~> 1.3.0', :subspecs => ['Extension']
end
// highlight-end
```
...
2. `PROJECT_ID`, `USERNAME`은 노티플라이 웹페이지의 [설정 페이지](https://notifly.tech/console/settings?environment=0)의 API, SDK 인증 정보 섹션에서 확인하실 수 있습니다. (initialize method에서 사용하는 값과 동일합니다.)
