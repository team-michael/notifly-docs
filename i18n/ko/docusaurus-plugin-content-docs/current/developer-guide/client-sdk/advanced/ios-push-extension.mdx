...
### 4. Test 
1. 이미지 첨부 테스트
    1-1. [노티플라이 SDK 연동 테스트 가이드](/developer-guide/client-sdk/sdk_test)를 참고하여, 이미지 첨부 후 푸시알림 테스트 발송을 진행합니다.
    1-2. 푸시 수신 시, 이미지가 정상적으로 표시되는지 확인합니다.

2. 푸시 수신 지표 수집 테스트 
    2-1. [노티플라이 SDK 연동 테스트 가이드](/developer-guide/client-sdk/sdk_test)를 참고하여, 푸시알림 테스트 발송을 진행합니다.
    2-2. [유저 리스트 페이지](https://www.notifly.tech/ko/console/users)에서 테스트한 유저의 상세 페이지로 이동합니다.
    2-3. 푸시 수신 지표가 정상적으로 수집되었는지 확인합니다.
