...
notifly.registerServiceWorker(NOTIFLY_VAPID_PUBLIC_KEY);
// highlight-end
...
참고: Service Worker가 웹에 등록되어 있을 때, 브라우저가 푸시를 사용자에게 보여줄 수 있는 권한이 설정되지 않은 경우에는 자동으로 권한을 요청하게 됩니다. 이 때, 사용자가 권한을 허용하지 않으면 푸시 알림을 수신할 수 없습니다. 더 자세한 내용은 [웹 푸시 알림 테스트](https://docs.notifly.tech/ko/developer-guide/client-sdk/sdk_test#5-%EC%9B%B9-%ED%91%B8%EC%8B%9C-%EC%95%8C%EB%A6%BC-%ED%85%8C%EC%8A%A4%ED%8A%B8)를 참고해 주세요.
