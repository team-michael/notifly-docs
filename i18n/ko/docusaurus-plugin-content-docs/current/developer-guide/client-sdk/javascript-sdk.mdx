...
```js title="Example ReactJS code for registering service worker -- index.js, App.js, _app.jsx or equivalent"
import notifly from 'notifly-js-sdk';
...
useEffect(() => {
    // highlight-next-line
...
- Notifly에서는 유저의 아이디 및 프로퍼티를 등록하지 않더라도 Notifly 자체적으로 임의의 ID로 유저를 등록하고 있습니다.
- 따라서 Notifly SDK 설치만으로도 해당 유저를 캠페인의 세그먼트로 포함시켜 메시지를 전송할 수 있지만, 유저 속성과 이벤트 기반의 효율적인 타겟팅을 위하여 유저 아이디 및 프로퍼티 등록을 권장하고 있습니다.
  - 타겟팅 활용 예시) Notifly에서는 채널 별 푸시 알림 수신 동의 여부를 유저 프로퍼티로 설정하여, 푸시 알림 전송 전에 필터링 할 수 있습니다.
- Notifly SDK 초기화 코드 추가를 마친 후 아래 예시 코드를 참고하여 유저의 아이디 및 프로퍼티 등록을 시작해 주세요.
  - [권장] Logout시에는 userId를 빈 값(null)으로 notifly.setUserId method를 호출하여 등록된 유저와 기기간의 연결을 끊어주세요. (예시 코드 참고)
