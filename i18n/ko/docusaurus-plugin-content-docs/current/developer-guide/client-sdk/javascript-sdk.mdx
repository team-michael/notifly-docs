...
| `pushSubscriptionOptions` | [NotiflyPushSubscriptionOptions](#웹-푸시-초기화-옵션) | No       | 웹 푸시 구독에 대한 옵션입니다. 자세한 설명은 아래 [웹 푸시 설정](#2-웹-푸시-설정) 섹션을 참고해 주세요. 미입력 시 웹 푸시 서비스를 이용하실 수 없습니다.                                      |
...
| Parameter           | Type    | Required | Description                                                                                               |
| ------------------- | ------- | -------- | --------------------------------------------------------------------------------------------------------- |
| `vapidPublicKey`    | string  | Yes      | 위에서 생성한 웹 푸시 인증서 키입니다. 2-1 에서 생성한 웹 푸시 인증서 키를 입력해 주세요                  |
| `askPermission`     | boolean | No       | 사용자가 웹페이지에 방문했을 때 알림 권한을 요청할 지에 대한 값입니다. default: true                      |
| `path`              | string  | No       | notifly-service-worker의 경로입니다. default: `'/notifly-service-worker.js'`.                             |
| `promptDelayMillis` | number  | No       | 사용자가 웹페이지에 방문했을 때 알림 권한 요청 팝업을 표시할때 까지의 지연 시간(ms) 입니다. default: 5000 |

2-2 에서 생성한 service worker를 위 [Notifly JS SDK 셋업](#1-notifly-js-sdk-셋업) 섹션의 `pushSubscriptionOptions` 초기화 옵션에 추가해 주세요.
