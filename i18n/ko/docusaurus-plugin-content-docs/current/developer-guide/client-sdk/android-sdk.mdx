...
Notifly Android SDK에서는 푸시를 발송하기 위해 Firebase Cloud Messaging을 활용하고 있습니다. [Firebase 프로젝트 셋업](https://firebase.google.com/docs/android/setup?authuser=0&hl=ko)이 되었는지 확인해 주세요. 
...
Notifly Android SDK를 다음과 같이 설치해 주세요. 

SDK의 현재 최신 버전: [![](https://jitpack.io/v/team-michael/notifly-android-sdk.svg)](https://jitpack.io/#team-michael/notifly-android-sdk) 
...
초기화는 Application 클래스에서 해주시는 것을 권장합니다. Activity 단계에서 초기화를 하시게 되면 다른 Activity들에 초기화가 적용되지 않을 수 있습니다.

아직 Application 클래스가 없다면 `AndroidManifest.xml`에서 `android:name`을 추가하고 Application 클래스 파일을 만들어 주세요.
...
다음 코드를 앱의 Application 클래스의 `onCreate`에 추가해 주세요.
...
```java title="Example Java code for setUserId"
import tech.notifly.Notifly
...
```java title="Example Java code for setUserProperties"
import tech.notifly.Notifly
...
Notifly.setUserProperties(context, userId);
```
...
```java title="Example Java code for trackEvent"
HashMap<String, Object> map = new HashMap<>();
...
- Q. 이미 Firebase Cloud Messaging을 사용 중인데 어떻게 해야 할까요?
    - A. Notifly Android SDK는 기존 앱에서 사용하고 있을 수 있는 Firebase Cloud Messaging과의 충돌을 방지하기 위한 처리를 이미 구현해 두었기 때문에 안전하게 사용하실 수 있습니다.
