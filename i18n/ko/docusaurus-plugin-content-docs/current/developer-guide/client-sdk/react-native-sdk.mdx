...
1. Xcode에서 `<Your_Project_Name>.xcworkspace`를 열어주세요.

- Xcode에서 `<Your_Project_Name>.xcworkspace`를 열지 않고 `<Your_Project_Name>.xcodeproj`를 열어서 빌드하면, Notifly SDK를 사용할 수 없습니다.
- `<Your_Project_Name>.xcworspace`는 Root 디렉토리의 ios 디렉토리에 있습니다.
...
- AppDelegate.mm ios 디렉토리의 `<ProjectName>` 폴더에 있습니다 (`<ProjectName>`을 실제 프로젝트 이름으로 대체하세요).
