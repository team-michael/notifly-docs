...
- Notifly에서는 자체적으로 몇 가지 이벤트를 로깅하고 있습니다. 이러한 이벤트들은 이벤트명 앞에 notifly\_\_ prefix가 붙어있습니다.
  - ex: 앱을 처음 시작했을 때, notifly\_\_session_start
- 본 섹션에서는, notifly\_\_session_start가 발생했을 경우, 인앱 메시지를 전송하는 테스트 캠페인을 생성해봄으로써 정상적으로 인앱 메시지 기능이 작동하는지를 테스트 합니다.
...
- 앱을 실행하면, notifly\_\_session_start이벤트가 자동으로 로깅되면서 인앱 메시지가 전송되며 아래와 같은 화면이 나타납니다.
