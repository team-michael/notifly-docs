...
`POST https://api.notifly.tech/projects/{projectId}/campaign_event_data/exports`
...
| end       | String | Yes      | 조회 종료일자 (yyyy-MM-ddTHH:mm:ss.sssZ, UTC 기준, 조회 시작일자로부터 최대 1달 이내) |
...
export 요청으로 얻은 export_id를 활용해 비동기로 생성된 Export 데이터를 페이지 당 최대 1000개 단위로 조회합니다. 데이터가 생성중이라면 요청에 대한 응답코드로 202를 반환합니다.
...
`GET https://api.notifly.tech/projects/{projectId}/campaign_event_data/exports/{exportId}/result?page={page}`
...
```http
GET https://api.notifly.tech/projects/{projectId}/campaign_event_data/exports/{exportId}/result?page=0
Content-Type: application/json
Authorization: {auth-key}
```
