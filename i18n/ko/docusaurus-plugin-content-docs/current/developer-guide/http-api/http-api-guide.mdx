...
| Parameter       | Type   | Required | Description                                                                                             |
| --------------- | ------ | -------- | ------------------------------------------------------------------------------------------------------- |
| projectd        | String | Yes      | Notifly 팀에서 제공드리는 project ID 입니다. 문의 사항은 contact@notifly.tech 으로 이메일 부탁드립니다. |
...
라는 구문이 들어있다면, 개인화는 실패하고 `님 안녕하세요` 라는 메시지가 발송됩니다. 이벤트 패러미터는 `user-id` 유형에 대해서도 사용 가능합니다.
