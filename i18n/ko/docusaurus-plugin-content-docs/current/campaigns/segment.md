...
2. 조건: 조건에 대한 자세한 정보는 [아래 섹션](/ko/campaigns/segment#조건의-종류)을 참고해주세요. 그룹 내의 조건들 끼리는 `AND` 연산자로 연결됩니다.
...
- [유저 등록](https://notifly.tech/users/register) 페이지에서 CSV 업로드, 또는 직접 입력하여 등록할 수 있습니다. [HTTP API - Set User Properties](/ko/http-api/http-api-guide#3-set-user-properties-endpoint)나 [Client SDK](/ko/client-sdk/react-native#2-how-to-set-user-information)를 통해서도 등록할 수 있습니다.
...
- 이벤트를 발생시키는 방법은 [캠페인 시작하기 - 이벤트 기반 발송](/ko/campaigns/basic#이벤트-기반-발송) 섹션을 참고해주세요.
...
- **새로운 세그먼트** 제작 기능은 노티플라이 시스템 내부에 등록된 유저를 사용하는 기능입니다. 따라서, **메시지 개인화를 할 때에 유저 및 디바이스 정보 뿐만 아니라 발생한 이벤트 파라미터(이벤트 기반 발송일 경우)를 모두 사용**하실 수 있습니다. 메시지 개인화에 대한 자세한 정보는 [메시지 개인화](/ko/campaigns/message#메시지-개인화) 섹션을 참고해주세요.
...
- **CSV 업로드** 기능은 노티플라이에 등록된 유저와 무관하게 발송됩니다. 따라서 메시지 개인화를 할 때에는, 노티플라이 데이터베이스에 등록된 유저 정보를 활용할 수 없습니다. CSV 파일 업로드를 했을 때는 **CSV 파일에 등록된 정보만으로 메시지 개인화를 할 수 있으며**, 메시지 개인화에 대한 자세한 정보는 [메시지 개인화](/ko/campaigns/message#메시지-개인화) 섹션을 참고해주세요.
