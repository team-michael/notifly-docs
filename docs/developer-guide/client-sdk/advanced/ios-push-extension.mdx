...
3. In TARGETS, select the created Extension, then add App Groups in the Signing & Capabilities tab, setting the group name as `group.notifly.<username>` (replace `<username>` with your actual username).
